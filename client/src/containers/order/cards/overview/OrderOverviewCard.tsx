...
const OrderOverviewCard = forwardRef<HTMLDivElement, OrderOverviewCardProps>(
	({ order, defaultOpen }, ref) => {
		...
		return (
			<main
				ref={ref}
				className='bg-popover p-4 mt-4 rounded-lg border shadow-sm'
			>
				<section
				...
				>
					<div className='flex gap-2 items-center'>
						<div
							className={`rounded-lg h-10 w-2 ${STATUS_COLOR[order.status]}`}
						/>
						<div>
						...
						>
							...
							{orderItems && orderItems.length > 3 && (
								<div className='w-[48px] h-[48px] p-2 bg-muted rounded-lg border text-center'>
									<h2 className='text-xl leading-4 mt-0.5'>
										+{orderItems.length - 3}
									</h2>
									<p className='text-primary text-[10px] opacity-90 leading-3'>
										items
									</p>
								</div>
							)}
