	...
	return (
		...
		>
			...
			<RadioGroup
				...
				onValueChange={value =>
					updateDetails({ paymentId: value === '-1' ? void 0 : +value })
				}
			>
				<div className='flex items-center space-x-2'>
					<RadioGroupItem id='receipt' value='-1' />
					<Label htmlFor='receipt' className='text-lg font-normal'>
				...
				{payments?.map(payment => (
					<div
						key={payment.id}
						className={cn('transition-all rounded-lg overflow-x-auto', {
					...
					>
						...
						{paymentId === payment.id && (
							<MagicCard className='my-2 mx-auto' payment={payment} />
						)}
					</div>
				))}
