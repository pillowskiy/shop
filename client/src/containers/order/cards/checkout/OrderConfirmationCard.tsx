...
export const OrderConfirmationCard: FC<OrderConfirmationCardProps> = ({
	...
	const { items, shippingId, paymentId, updateDetails } =
		useContext(OrderCheckoutContext)
	...
	const { mutate: createShipping } = useMutation(
		['create shipping'],
		({ data }: { data: CreateShippingData }) => {
	...
	return (
		...
		>
			...
			<AlertDialog>
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							This shipping method was not found. Would you like to save it for
							future orders?
						</AlertDialogDescription>
					</AlertDialogHeader>
					{data && (
						<AlertDialogFooter>
							...
							<AlertDialogCancel
								onClick={() =>
									createShipping({ data: { ...data, temp: true } })
								}
							>
								Leave it as a one-time use
							</AlertDialogCancel>
							<AlertDialogAction
								onClick={() => createShipping({ data: { ...data } })}
							>
								Save
							</AlertDialogAction>
						</AlertDialogFooter>
			...
			<p className='mt-2 text-xs font-medium'>
				...
				<Anchor className='opacity-90' href='#'>
					Regulations on the personal data
				</Anchor>
				{' and '}{' '}
				<Anchor className='opacity-90' href='#'>
					User agreements
