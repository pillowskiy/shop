...
export const OrderDetailedInfo: FC<OrderDetailedInfoProps> = ({
	order,
	items
}) => {
		...
		<main className='flex flex-col sm:flex-row gap-4 mt-4'>
			<section className='w-full md:w-4/12 px-2'>
				...
				<div className='mt-4'>
					...
					<InfoRow title='Phone'>{order.shipping.phone}</InfoRow>
					<InfoRow title='Email'>{profile.email}</InfoRow>
					{order.shipping.temp && (
						<HoverInfoCard
						...
						>
							<div className='mt-4 flex space-x-1 items-start md:hover:underline transition-all cursor-pointer w-fit'>
								<p className='text-sm text-muted-foreground'>
									Temp shipping method
								</p>
			...
			<section className='w-full md:w-8/12 px-2'>
				<Accordion type='single' collapsible>
					<AccordionItem className='mb-4' value='description'>
						<AccordionTrigger className='font-medium text-xl select-none p-0 pb-2'>
							Items
						</AccordionTrigger>
						<AccordionContent className='max-h-[300px] overflow-y-auto rounded-lg p-4 pb-0 my-4 border'>
							{items.map(item => (
								<CartReadonlyItemCard
									key={item.id}
									item={{
				...
				<InfoRow title='Delivery'>at the carrier tariffs</InfoRow>
