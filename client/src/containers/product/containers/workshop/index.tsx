...
export const ProductWorkShop: FC<ProductWorkShopProps> = ({ product }) => {
	const [newProduct, setNewProduct] = useState<UpdateProductData>(
		getInitialProductState(product)
	)
	const [errors, setErrors] = useState<UpdateProductDataErrors>(
	...
	const { mutate: upsertMutation } = useMutation(
			...
			ProductService.upsert(formData, product?.id),
		{
			...
			onSuccess: ({ data }) => {
				toast(
					buildToast('product.workshop.success', {
	...
	return (
		<WorkShopContext.Provider value={{ product, newProduct, errors }}>
			<section className='grid md:grid-cols-2 gap-4 mt-4 md:pb-0 max-w-[1080px]'>
				{!isEquals(getInitialProductState(product), newProduct) && (
					<UnsavedChangesDialog />
				)}
				<GeneralCard onConfirm={handleUpload} updateProduct={updateProduct} />
				<section className='grid grid-rows-[2fr 1fr] gap-4 row-span-2'>
					<ImageUploadCard
						...
						deleteImage={src => {
							// TEMP
							setImages(images =>
								images.filter(({ preview }) => preview !== src)
							)
							updateProduct({
								images: newProduct.images.filter(image => image !== src)
							})
						}}
					/>
					<TextareaCard
