...
export const ProductHorizontalInfo: FC<ProductHorizontalInfoProps> = ({
	product
}) => {
	return (
		<Link
			className='flex w-full md:w-11/12 h-[inherit] items-center'
				...
				<div className='flex'>
					<p className='hover:underline transition-all'>
						{product.name.length > 22
							? product.name.slice(0, 24).concat('..')
							: product.name}
					</p>
					<QuantityBadge quantity={product.quantity} />
				</div>
				<p className='text-xs hidden md:block'>
					{product.description?.slice(0, 48).concat('..')}
				</p>
			</section>
		</Link>
	)
